...
(atmel =function($){
    ...
    $.fn.showContextMenu = function(id, e){
    	  // Check for the modifier if any.
		if (typeof _menus[id].modifier == 'string' && !e[_menus[id].modifier]) {alert('first');return;}
       ...
       if(_menus[id].openBelowContext)
        {
        	alert('_menus[id].openBelowContext')
		...
		//	alert('mouseover.jeegoocontext,e:'+e)
	...
	$.fn.jeegoocontext = function(id, options){
		//alert('$.fn.jeegoocontext(id, options) 102')
        if(!_global) _global = {};
        if(!_menus) _menus = {};
        ...
        //alert('267,eventType='+eventType);
     //  alert();
       //alert(_menus[id].livequery ? 'livequery' : 'bind')
        return false;
        return this[_menus[id].livequery ? 'livequery' : 'bind'](eventType, function(e){
        	
        	return;
        	//alert(_menus[id].livequery ? 'livequery' : 'bind')
        	//alert('id='+id+','+e.pageX +':'+e.pageY )
            // Check for the modifier if any.
			if (typeof _menus[id].modifier == 'string' && !e[_menus[id].modifier]) {alert('first');return;}
