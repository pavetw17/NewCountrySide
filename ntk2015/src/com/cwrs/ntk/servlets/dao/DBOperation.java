...
/*import com.cwrs.ntk.dataobject.OTblSau;
import com.cwrs.ntk.dataobject.TblRuongGieocay;*/
...
/*	public int execInsertUpdatedMultiRecords2DBByClient(String sql, String condition, String strListValues, String strListIDs){
	...
	*/
	 ...
	 * 
	 */
	public String execInsertUpdatedMultiRecords2DBByClient(String sql, String condition, String strListValues, String sqlTypeOrder, String strListIDs) throws ParseException{
		...
		if (strListIDs == null)
			System.out.println("strListIDs is NULL");
				...
				for (int i = 0; i< lstValues.length; i++){
					String[] values = lstValues[i].split(Definitions.SQL_VALUES_SEP);
						for (int j = 0; j < values.length; j++){
							...
							} else  if (typeOrder[j].equals(Definitions.SQL_TYPE_BOOLEAN)){
								preStmt.setBoolean(j + 1, Boolean.valueOf(values[j]));
							} else  if (typeOrder[j].equals(Definitions.SQL_TYPE_TIME)){
								SimpleDateFormat sdf = new SimpleDateFormat("hh:mm");
								Date date = sdf.parse(values[j]);
								preStmt.setTime(j + 1, new java.sql.Time(date.getTime()));
							} else {
 ...
 */
	...
	public  void closeEverything(ResultSet rs, Statement stmt) {
		if (rs != null) {
			try {
				...
				System.out.println(" closeEverything error" + e.toString() );
			}
		}
		if (stmt != null) {
			try {
	...
	public  void closeEverything(Statement stmt ) {
		if (stmt != null) {
			try {
				stmt.close();
			} catch (SQLException e) {
	...
	public  void closeEverything(ResultSet rs, PreparedStatement pstmt) {
		if (rs != null) {
			try {
				rs.close();
			} catch (SQLException e) {
	...
	public  void closeEverything(PreparedStatement pstmt) {
		if (pstmt != null) {
			try {
				pstmt.close();
			} catch (SQLException e) {
