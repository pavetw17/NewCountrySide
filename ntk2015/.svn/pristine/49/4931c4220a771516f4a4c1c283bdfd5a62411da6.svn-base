...
tinymce.util.Quirks = function(e) {
	...
	function k() {
		function K(N) {
			...
			P = q.getParent(L.startContainer, q.isBlock);
			tinymce.each(q.select(
					"span.Apple-style-span,font.Apple-style-span", P),
					function(Q) {
						var R = I.getBookmark();
						if (O) {
	...
	function E() {
		...
		function K() {
			return !I.isCollapsed()
					&& q.getParent(I.getStart(), q.isBlock) != q.getParent(I
							.getEnd(), q.isBlock)
		}
		function M(N, O) {
			O.preventDefault();
		...
		c.addNodeFilter("pre", function(K, M) {
			...
			while (N--) {
				...
				while (L--) {
					...
					O = Q.prev;
					if (O && O.type === 3
							&& O.value.charAt(O.value - 1) != "\n") {
						O.value += "\n"
					} else {
						Q.parent.insert(new tinymce.html.Node("#text", 3), Q,
								true).value = "\n"
					}
				}
			}
		...
		e.onKeyDown.add(function(Q, R) {
			...
			if (!H(R) && (P || R.keyCode == x) && !n.modifierPressed(R)) {
				...
				if (L.nodeType == 3 && L.nodeValue.length > 0
					...
					Q.getDoc().execCommand(P ? "ForwardDelete" : "Delete",
							false, null);
					L = I.getRng().startContainer;
					S = L.previousSibling;
					if (S && S.nodeType == 1 && !q.isBlock(S) && q.isEmpty(S)
							&& !nonEmptyElements[S.nodeName.toLowerCase()]) {
						q.remove(S)
					}
					q.remove("__tmp")
				}
	...
	function f() {
		e.onKeyDown.add(function(O, P) {
			...
			while (N && N.parentNode && N.parentNode.firstChild == N
					&& N.parentNode != K) {
				N = N.parentNode
			}
	...
	function h() {
		var M, L, ac, K, X, aa, Y, ab, N, O, Z, V, U, W = document, S = e
				.getDoc();
		...
		function Q(ag) {
			...
			V = V < 5 ? 5 : V;
			U = U < 5 ? 5 : U;
			if (n.modifierPressed(ag)
					|| (ac.nodeName == "IMG" && X[2] * X[3] !== 0)) {
				V = Math.round(U / O);
				U = Math.round(V * O)
			}
			q.setStyles(K, {
		...
		function ad() {
			function ae(af, ag) {
				if (ag) {
					if (ac.style[af]
							|| !e.schema.isValid(ac.nodeName.toLowerCase(), af)) {
						q.setStyle(ac, af, ag)
					} else {
	...
	b.html.DomParser = function(g, h) {
		...
		function j(n) {
			...
			for (p = 0; p < n.length; p++) {
				...
				x = [ q ];
				for (y = q.parent; y && !h.isValidChild(y.name, q.name)
						&& !t[y.name]; y = y.parent) {
					x.push(y)
				}
				if (y && x.length > 1) {
					...
					y = x[0];
					if (y.isEmpty(k) || y.firstChild === y.lastChild
							&& y.firstChild.name === "br") {
						y.empty().remove()
					}
				} else {
					if (q.parent) {
						...
						if (h.isValidChild(q.parent.name, "div")
								&& h.isValidChild("div", q.name)) {
			...
			.create(
					"tinymce.Editor",
					{
									...
									function(t, u) {
										...
										while (v--) {
											...
											if (!y.attributes.map[x]) {
												if (u === "style") {
													y.attr(x, A.serializeStyle(
															A.parseStyle(z),
															y.name))
												} else {
													y.attr(x, n.convertURL(z,
															u, y.name))
												}
											}
										}
						...
						translate : function(m) {
							var o = this.settings.language || "vi", n = k.i18n;
							if (!m) {
								return ""
							}
							return n[o + "." + m]
					...
					"InsertUnorderedList,InsertOrderedList" : function(x) {
						...
						return v
								&& (x === "insertunorderedlist"
										&& v.tagName === "UL" || x === "insertorderedlist"
										&& v.tagName === "OL")
					}
				}, "state");
		u({
...
(function(a) {
	a.Formatter = function(aa) {
		var Q = {}, T = a.each, c = aa.dom, r = aa.selection, t = a.dom.TreeWalker, N = new a.dom.RangeUtils(
				c), d = aa.schema.isValidChild, A = a.isArray, H = c.isBlock, m = aa.settings.forced_root_block, s = c.nodeIndex, G = "\uFEFF", e = /^(src|href|style)$/, X = false, C = true, P, D, x = c.getContentEditable;
		function I(ab) {
		...
		function p(ab, am, ae) {
			...
			function ao(aA) {
				...
				at = c.getRoot();
				function ay(aB) {
					return aB.nodeName == "BR"
							&& aB.getAttribute("data-mce-bogus")
							&& !aB.nextSibling
				}
...
(function(b) {
	...
	b.EnterKey = function(f) {
		var i = f.dom, e = f.selection, d = f.settings, h = f.undoManager, c = f.schema
				.getNonEmptyElements();
		function g(A) {
			var v = e.getRng(true), G, j, z, u, p, M, B, o, k, n, t, J, x, C;
			function E(N) {
				return N && i.isBlock(N)
						&& !/^(TD|TH|CAPTION|FORM)$/.test(N.nodeName)
						&& !/^(fixed|absolute)/i.test(N.style.position)
						&& i.getContentEditable(N) !== "true"
			}
			function F(O) {
				var N;
			...
			function q(Q) {
				...
				if (z.nodeName === "TABLE"
						|| (z.previousSibling && z.previousSibling.nodeName == "TABLE")) {
					return (C && !Q) || (!C && Q)
				}
				P = new a(z, p);
				if (z.nodeType == 3) {
			...
			function I(O) {
				...
				if (!b.isIE) {
					...
					N = O.lastChild;
					if (!N
							|| (/^(left|right)$/gi.test(i.getStyle(N, "float",
									true)))) {
						i.add(O, "br")
					}
				}
