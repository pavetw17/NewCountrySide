...
(function($) {
	...
	$.mbExtruder= {
		...
		getOptions:function(){
			//alert($(this).get(0).options)
			return $(this).get(0).options;
		},
		...
		callback:function () {
			var extruder= $(this);
			setTimeout(function() {
				extruder.find('.content').removeAttr( "style" ).fadeOut();
			}, 1000 );
		},


		openMbExtruder:function(delay, c){
			var extruder= $(this);
			if(extruder.attr("isOpened") == 'true') return;//tuanna added this
			...
			if (position=="top" || position=="bottom"){
				...
				if (delay == undefined){
					//var options = { percent: 300 };
					//extruder.find('.content').show( "scale", options, 500, this.callback);
					extruder.find('.content').animate({ width: opt.width}, opt.slideTimer);
				} else if (delay == 0){
					
					extruder.find('.content').css({ width: opt.width});
				} else {
					
					extruder.find('.content').animate({ width: opt.width}, delay);
				}
				if(opt.onExtOpen) opt.onExtOpen();
		...
		closeMbExtruder:function(delay){
			var extruder= $(this);
			extruder.removeAttr("isOpened");
			var opt= extruder.get(0).options;
			...
			if (opt.position=="top" || opt.position=="bottom"){
				...
				extruder.find('.content').animate({ width: 1 }, (delay == undefined?opt.slideTimer:delay),function(){
		...
		isOpen:function(){
			var extruder= $(this);
			if(extruder.attr("isOpened") == 'true')
				return true;
			else
				return false;	
		},
		hideFlap:function(){
			var extruder= $(this);
			extruder.find(".flap").hide();
			
		},
		showFlap:function(){
			var extruder= $(this);
			extruder.find(".flap").show();
	...
	$.fn.disableExtruderVoice=function(){
		...
		label.addClass("disabled").css("cursor","pointer");
	...
	$.fn.isOpen=$.mbExtruder.isOpen;
	$.fn.hideFlap=$.mbExtruder.hideFlap;
	$.fn.showFlap=$.mbExtruder.showFlap;
	$.fn.self=$.mbExtruder.self;
	$.fn.callback=$.mbExtruder.callback;
	$.fn.getOptions=$.mbExtruder.getOptions;
